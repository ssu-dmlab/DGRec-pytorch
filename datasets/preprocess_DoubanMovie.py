...
DATA_NAME = 'music'
PATH_TO_DATA = '/Users/kimtaesu/PycharmProjects/DGRec-pytorch/datasets/Douban/'

SOCIAL_NETWORK_FILE = PATH_TO_DATA + 'socialnet/socialnet.tsv'
RATING_FILE = PATH_TO_DATA + DATA_NAME + '/douban_'+DATA_NAME+'.tsv'
max_length = 30
...
def split_data(day): #split data for training/validation/testing.
    ...
    with open(PATH_TO_DATA + DATA_NAME+'/user_id_map.tsv', 'w') as fout:
        for k, v in user_map.items():
            fout.write(str(k) + '\t' + str(v) + '\n')
    with open(PATH_TO_DATA + DATA_NAME+'/item_id_map.tsv', 'w') as fout:
    ...
    train_tr.to_csv(PATH_TO_DATA+DATA_NAME+'/train.tsv', sep='\t', index=False)
    valid.to_csv(PATH_TO_DATA+DATA_NAME+'/valid.tsv', sep='\t', index=False)
    test.to_csv(PATH_TO_DATA+DATA_NAME+'/test.tsv', sep='\t', index=False)
    df_net.to_csv(PATH_TO_DATA+DATA_NAME+'/adj.tsv', sep='\t', index=False)
    with open(PATH_TO_DATA+DATA_NAME+'/latest_sessions.txt', 'w') as fout:
        for idx in range(num_users):
