...
class DGRec(torch.nn.Module):
    ...
    def forward(self, feed_dict, labels, graph):
        ...
        #logits = torch.argmax(torch.swapaxes(logits, 1, 2), 1)


        #print(logits.shape)
        #print(np.array(feed_dict['mask_y']).shape)
        logits *= torch.unsqueeze(torch.LongTensor(feed_dict['mask_y']), 2)
        ...
        loss = E.cross_entropy(logits, labels)
    ...
    def predict(self, feed_dict, graph):
        ...
        logits *= torch.LongTensor(feed_dict['mask_y'])
